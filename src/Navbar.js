			...
			return (
				<div className="links-container">
					...
					<div className="label" onClick={() => setToggleProjects(!toggleProjects)}>projects</div>
						<div className="links-container" style={toggleProjects ? {height: props.projects.length * cellHeight} : {height: "0px"}}> 
							{
								props.projects.map((project, i) => {
									if(project === props.currentContent){
										return <div className="sub-label selected" style={{"height" : cellHeight}} onClick={() => selectProject(project)} key={i}>{project.title}</div>
									}
									return <div className="sub-label" style={{"height" : cellHeight}} onClick={() => selectProject(project)} key={i}>{project.title}</div>
								})
							}
						</div>
					<div className="label" onClick={() => setToggleContactLinks(!toggleContactLinks)}>contact/links</div>
						<div className="links-container" style={toggleContactLinks ? {height: 2 * cellHeight} : {height: "0px"}}> 
							<div className="contact">
								<div className="sub-label" style={{"height" : cellHeight}} onClick={() => window.open('http://github.com/tennyx', '_blank', 'noopener,noreferrer')}>
								...
								<div className="sub-label" style={{"height" : cellHeight}} onClick={() => copyText()}>
